...
test('commits include author date (YYYY-MM-DD)', () => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'dhnd-git-'));
  ...
  fs.writeFileSync(path.join(dir, 'a.txt'), '1');
  g(['add', '-A']);
  g(['commit', '-m', 'dated'], { GIT_COMMITTER_DATE: '2026-01-08T10:00:00Z', GIT_AUTHOR_DATE: '2026-01-08T10:00:00Z' });
  const commits = commitsSince(dir, '2020-01-01T00:00:00Z');
  assert.equal(commits[0].date, '2026-01-08');
