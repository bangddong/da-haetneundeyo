...
function collectFallbackCommits(cwd, w, digest, config, state, merged) {
  const repos = new Set();
  ...
  for (const repoDir of repos) {
    const author = config.gitAuthor ?? state.authors[repoDir] ?? repoAuthorEmail(repoDir);
    if (author && !config.gitAuthor) state.authors[repoDir] = author;
...
function collectCommits(digest, config, state) {
  ...
  for (const cwd of cwds) {
    ...
    const author = config.gitAuthor ?? state.authors[cwd] ?? repoAuthorEmail(cwd);
    if (author && !config.gitAuthor) state.authors[cwd] = author; // 저장소별 캐시 (git 스폰 절약)

    const commits = commitsSince(cwd, w.start, w.end, author);
    if (commits === null) {
      if (collectFallbackCommits(cwd, w, digest, config, state, merged)) anySucceeded = true;
      continue;
    }
    anySucceeded = true;
...
export function captureTranscript({ sessionId, transcriptPath, complete = false }, env = process.env) {
  ...
  collectCommits(digest, config, state);
