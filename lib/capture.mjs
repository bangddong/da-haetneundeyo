...
import { parseLine, applyRecords, emptyDigest, finalizeKind, finalizeProject, finalizeArchetype, EDIT_TOOL_NAMES } from './transcript.mjs';
import { commitsSince, repoAuthorEmail, repoToplevel } from './git.mjs';
...
// 세션이 거쳐간 cwd(=digest.cwdWindows)마다 개별적으로 커밋을 조회해 병합한다 (#3). 하위 경로 cwd
// 두 개가 같은 저장소를 가리켜 동일 커밋이 중복 조회될 수 있으므로 hash 기준으로 dedupe한다.
// 일부 cwd가 비 git(commitsSince → null)이어도 나머지 cwd 결과는 유지하고, 전부 null이면 기존
// digest.commits를 그대로 둔다(기존 계약 유지: git 조회 실패로 이전에 채워진 커밋을 지우지 않음).
function isUnder(file, dir) {
  const rel = path.relative(dir, file);
  return rel !== '' && rel !== '..' && !rel.startsWith('..' + path.sep) && !path.isAbsolute(rel);
}

// cwd가 git repo가 아닌 창(멀티 프로젝트 워크스페이스 루트 등)의 폴백: 그 cwd 하위에서 편집된
// 파일들의 dirname으로 repo 루트를 추론해, 같은 시간 창으로 커밋을 수집한다 (#11). 추론 실패
// (repo 밖 파일·삭제된 경로)는 조용히 스킵 — 캡처를 막지 않는다.
function collectFallbackCommits(cwd, w, digest, config, authorCache, merged) {
  const repos = new Set();
  const seenDirs = new Set(); // dirname 단위로 rev-parse 1회 (git 스폰 절약)
  for (const f of digest.filesEdited) {
    if (!isUnder(f, cwd)) continue;
  ...
  for (const repoDir of repos) {
    const author = resolveAuthor(repoDir, config, authorCache);
    const commits = commitsSince(repoDir, w.start, w.end, author);
    if (commits === null) continue;
    anySucceeded = true;
    const repo = path.basename(repoDir);
    for (const c of commits) {
      if (!merged.has(c.hash)) merged.set(c.hash, { ...c, repo });
    }
  }
  return anySucceeded;
}

// author 이메일 해석 결과를 프로세스 수명 동안만 캐시한다. state.json에 영구 저장하지 않는
// 이유: 저장소의 git user.email이 바뀌어도 캐시가 눈치채지 못해, 그 저장소 커밋이 이후 모든
// 보고에서 조용히 누락된다 (--author 필터 불일치). 스윕/백필은 캐시 하나를 전 세션에 공유해
// 저장소당 git config 조회를 1회로 묶고(백필 스폰 누적 방지), 매 실행은 새 캐시로 시작해
// 이메일 변경을 다음 실행에 즉시 반영한다.
function resolveAuthor(repoDir, config, authorCache) {
  if (config.gitAuthor) return config.gitAuthor;
  if (authorCache.has(repoDir)) return authorCache.get(repoDir);
  const author = repoAuthorEmail(repoDir);
  authorCache.set(repoDir, author);
  return author;
}

function collectCommits(digest, config, authorCache) {
  const cwds = Object.keys(digest.cwdWindows);
  ...
  for (const cwd of cwds) {
    const w = digest.cwdWindows[cwd];
    const author = resolveAuthor(cwd, config, authorCache);

    const commits = commitsSince(cwd, w.start, w.end, author);
    if (commits === null) {
      if (collectFallbackCommits(cwd, w, digest, config, authorCache, merged)) anySucceeded = true;
      continue;
    }
    anySucceeded = true;
  ...
  if (anySucceeded) digest.commits = [...merged.values()];
}

export function captureTranscript({ sessionId, transcriptPath, complete = false }, env = process.env, authorCache = new Map()) {
  const state = loadState(env);
  delete state.authors; // 구버전 상태의 영구 author 캐시 제거 (이제 프로세스 스코프)
  ...
  finalizeProject(digest);
  collectCommits(digest, config, authorCache);
  if (complete) digest.completed = true;
  finalizeKind(digest);
  finalizeArchetype(digest);
...
export function sweepProjects(env = process.env, { sinceMs, days } = {}) {
  ...
  const cutoff = sinceMs ?? (days != null
    ...
    : state.lastSweepMs);
  const config = loadConfig(env);
  // 스윕 전체가 author 캐시 하나를 공유한다 — 같은 저장소를 건드린 세션이 여럿이어도
  // git config user.email 조회는 저장소당 1회 (백필 시 git 스폰 누적 방지).
    ...
    for (const proj of projects) {
      ...
      for (const f of files) {
        ...
        try {
          ...
          const d = captureTranscript({ sessionId, transcriptPath: full }, env, authorCache);
          if (d) processed += 1;
