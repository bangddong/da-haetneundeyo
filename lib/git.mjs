...
// timeout은 git이 행에 걸렸을 때의 안전망이다. 너무 짧으면 느린 머신(AV 스캔 등)의 부하 상황에서
// 정상 스폰이 타임아웃돼 커밋이 조용히 누락된다 — 5s에서 상향 (실측: 부하 시 스폰 5s+ 관측).
const GIT_OPTS = { encoding: 'utf8', timeout: 15_000, stdio: ['ignore', 'pipe', 'ignore'] };
...
// 디렉토리가 속한 repo 루트를 돌려준다. 비 git 디렉토리·존재하지 않는 경로 등 모든 실패는 null.
// 워크스페이스 루트(비 repo) cwd 창에서 편집 파일 경로로 하위 repo를 추론하는 폴백에 쓰인다 (#11).
export function repoToplevel(dir) {
  try {
    const out = execFileSync('git', ['-C', dir, 'rev-parse', '--show-toplevel'], GIT_OPTS).trim();
    return out || null;
  } catch {
    return null;
  }
