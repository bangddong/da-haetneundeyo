...
// ISO 타임스탬프를 로컬 날짜(YYYY-MM-DD)로 변환한다. 저장 키는 UTC 날짜(dayOf)를 유지하되,
// 조회·표시는 사용자 체감(로컬 자정 기준)을 따라야 하므로 이 변환을 거친다 (#4 — KST에서
// 자정~오전 9시 세션이 "오늘" 조회에서 누락되던 버그). offsetMinutes 미지정 시 머신 로컬
// 오프셋(DST 반영)을 쓰고, 테스트·특수 환경은 DHND_UTC_OFFSET_MIN(env)으로 고정할 수 있다.
export function localDay(iso, offsetMinutes) {
  const d = new Date(iso);
  const off = offsetMinutes ?? -d.getTimezoneOffset();
  return new Date(d.getTime() + off * 60_000).toISOString().slice(0, 10);
}

function envOffset(env) {
  const n = Number(env?.DHND_UTC_OFFSET_MIN);
...
export function findDigest(sessionId, day, env) {
  // 파일 키는 UTC 날짜지만 스킬이 사용자에게 보여주는 날짜는 로컬이라, 사용자가 넘긴 --day와
  // 실제 파일이 ±1일 어긋날 수 있다 (양수 오프셋은 -1, 음수 오프셋은 +1 방향). sessionId가
  // 유일하므로 인접일 폴백은 안전하다.
  for (const dd of [day, shiftDay(day, -1), shiftDay(day, 1)]) {
    for (const line of readLines(dayFilePath(dd, env))) {
...
export function readRange(fromDay, toDay, env) {
  const off = envOffset(env);
  const out = [];
  const cur = new Date(`${fromDay}T00:00:00Z`);
  cur.setUTCDate(cur.getUTCDate() - LOOKBACK_DAYS);
  const end = new Date(`${toDay}T00:00:00Z`);
  // 음수 오프셋 타임존(미주 등)에서는 로컬 toDay 저녁 세션이 UTC 다음날 파일에 적재되므로
  // 파일 읽기 창을 하루 더 확장한다. 포함 여부는 아래 로컬 날짜 겹침 판정이 결정한다.
  ...
  while (cur <= end) {
    ...
    for (const line of readLines(dayFilePath(day, env))) {
      const d = parseDigestLine(line);
      if (!d) continue;
      // 겹침 판정은 로컬 날짜 기준 (#4): 세션의 [로컬 시작일, 로컬 종료일] 구간이
      // [fromDay, toDay]와 겹치면 포함한다.
      const startDay = localDay(d.start ?? `${day}T00:00:00Z`, off);
      const endDay = localDay(d.end ?? d.start ?? `${day}T00:00:00Z`, off);
      if (endDay < fromDay || startDay > toDay) continue;
      out.push(d);
    }
