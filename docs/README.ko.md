...
멀티 프로젝트 워크스페이스 루트(자체는 git 저장소가 아니지만 하위에 여러 저장소를 둔 디렉토리)에서 진행한 세션도 지원합니다: 세션의 cwd가 저장소가 아니면, 편집한 파일 경로로 어느 하위 저장소를 건드렸는지 추론해 같은 시간 창으로 해당 저장소들의 커밋을 수집합니다.
